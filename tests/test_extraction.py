...
from tests.fixtures_docs import create_all_docs, write_manifest
...
# Eagerly create docs + manifest so parametrize works at collection time
TEST_DATA_DIR.mkdir(parents=True, exist_ok=True)
create_all_docs(TEST_DATA_DIR)
write_manifest(MANIFEST_PATH, TEST_DATA_DIR)
...
def test_extraction_via_api_per_manifest(doc, monkeypatch):
    ...
    if not doc['should_parse']:
        # Corrupt files may return 422 (no content) or 200 (partial extraction
        # from garbage) depending on engine behavior. Both are acceptable.
        assert resp.status_code in (200, 422), f"Unexpected status for corrupted {file_path.name}: {resp.status_code}"
        return
    ...
    assert body.get('triage') is not None
    assert 'engine' in body['triage']
    assert 'file_type' in body['triage']
